...
def search_with_duckduckgo(query: str):
    """
    Search with DuckDuckGo API and return the results in Bing API format.
    """
    url = f"{DUCKDUCKGO_API_ENDPOINT}?q={query}&max_results={REFERENCE_COUNT}"
    ...
    json_content = response.json()
    logger.debug(f"DuckDuckGo API response: {json_content}")
...
class RAG(Photon):
    ...
    def init(self):
        ...
        if self.backend == "LEPTON":
            ...
            self.search_function = search_with_duckduckgo
        else:
            raise RuntimeError("Backend must be LEPTON, BING, GOOGLE, SERPER, SEARCHAPI or DUCKDUCKGO.")
